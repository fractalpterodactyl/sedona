...
static bool doApplyRs485Setting(struct SerialPortLinux *h, int rs485, int rts)
{
			...
			int     rs485)
{
	...
	if (rs485)
		if (! doApplyRs485Setting(h, rs485, rts)) return false;
	else
...
Cell serial_SerialPortNative_doCloseNative(SedonaVM* vm, Cell* params)
{
	...
	int fd = portHandles[portNum].fd;
	if (fd) {
		close(fd);
		portHandles[portNum].fd = 0;
	}
	return zeroCell;
}
